...
{
    using BvhNode = typename BvhNode;
    ...
    std::pair<size_t, size_t> cluster(const BvhNode *bvh_restrict input, BvhNode *bvh_restrict output, size_t *bvh_restrict neighbors,
            ...
            for (size_t i = begin; i < end; ++i)
            {
                ...
                if (neighbors[j] == i)
                {
                    if (i < j)
                    {
                        ...
                        unmerged_node.boundingBox->setNode(std::make_shared<BvhNode>(unmerged_node));
                    }
                }
                else
                {
...
public:
    ...
    void build(const BoundingBox &global_bbox, std::vector<std::shared_ptr<BoundingBox>> &bboxes)
    {
...
#pragma omp parallel for
        ...
        auto nodes = std::make_unique<BvhNode[]>(node_count);
        auto nodes_copy = std::make_unique<BvhNode[]>(node_count);
...
#pragma omp parallel for
        for (size_t i = 0; i < primitive_count; ++i)
        {
            ...
            node.boundingBox = bboxes[primitive_indices[i]];
    ...
    void test(IndexType previousIdx, IndexType currentIdx)
    {
        ...
        std::shared_ptr<BvhNode> ptr(&bvh.nodes[currentIdx]);
        bvh.nodes[currentIdx].boundingBox->setNode(ptr);
